...
export const BotBubble: React.FC<BotBubbleProps> = ({ message, avatar }) => {
  const isShort = (message.text || '').length <= 40;

  return (
    <div className="flex justify-start mb-2 animate-fade-in gap-2 items-end">
      <img src={avatar} alt="Old Mutual" className="w-5 h-5 sm:w-6 sm:h-6 object-contain flex-shrink-0" />
